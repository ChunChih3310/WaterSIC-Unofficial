...
from watersic.debug.o_proj_residual import run_o_proj_residual_debug
from watersic.utils.runtime import load_config, prepare_runtime, resolve_torch_device, select_runtime_device


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the layer1 o_proj residual-correction debug sweep")
...
def main() -> None:
    ...
    seed = int(debug_config.get("prefix_quant", {}).get("layer", {}).get("seed", 0))
    ...
    result = run_o_proj_residual_debug(
        model_config,
        debug_config,
        device=device,
        logger=logger,
    ...
    logger.info("o_proj residual debug complete: %s", result["summary_markdown_path"])
