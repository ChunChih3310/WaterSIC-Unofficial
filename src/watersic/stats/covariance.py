...
class SecondMomentAccumulator:
    ...
    def state_dict(self) -> dict[str, object]:
        return {
            ...
            "dim_out": int(self.dim_out if self.dim_out is not None else self.dim_in),
            "dtype": str(self.dtype).replace("torch.", ""),
            "sum": self._sum.clone(),
            "weight_sum": self._weight_sum.clone(),
        }

    @classmethod
    def from_state_dict(cls, state: dict[str, object]) -> "SecondMomentAccumulator":
        dtype_name = str(state["dtype"])
        dtype = getattr(torch, dtype_name)
        accumulator = cls(
            dim_in=int(state["dim_in"]),
            dim_out=int(state["dim_out"]),
            dtype=dtype,
        )
        accumulator._sum.copy_(state["sum"].to(dtype))
        accumulator._weight_sum.copy_(state["weight_sum"].to(dtype))
        return accumulator
