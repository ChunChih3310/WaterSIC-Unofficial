...
from watersic.quant.checkpoint import CheckpointConfig
...
def build_sequential_config(quant_config: dict) -> SequentialQuantizationConfig:
    checkpoint_cfg = quant_config.get("checkpoint", {})
    return SequentialQuantizationConfig(
        ...
        checkpoint=CheckpointConfig(
            ...
            dir=str(checkpoint_cfg.get("dir", "outputs/checkpoints")),
            resume=str(checkpoint_cfg.get("resume", "auto")),
            strict_git_commit=bool(checkpoint_cfg.get("strict_git_commit", True)),
            keep_after_completion=bool(checkpoint_cfg.get("keep_after_completion", True)),
            save_collection_every_batches=int(checkpoint_cfg.get("save_collection_every_batches", 32)),
        ),
    )
...
def run_full_experiment(
    ...
    *,
    ...
    report_metadata = {
        ...
        "calibration_split": calibration_config.split,
    ...
    run_report, artifact_dir = quantize_model_sequential(
        ...
        quant_config_snapshot=quant_config,
    ...
    run_report.extras["checkpoint_enabled"] = bool(sequential_config.checkpoint.enabled)
    run_report.extras["checkpoint_resume_mode"] = sequential_config.checkpoint.resume
    run_report.extras["checkpoint_dir"] = (
        str(Path(sequential_config.checkpoint.dir) / quant_config["run_name"]) if sequential_config.checkpoint.enabled else None
    )
