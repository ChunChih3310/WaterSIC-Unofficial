...
@dataclass(frozen=True)
class ObjectiveQuantizationResult:
    quantized_weight: torch.Tensor
    selected_c: float
...
class PreparedLayerProblem:
    ...
    dead_features: DeadFeatureReport
    compensation_matrix: torch.Tensor | None
    rescaler_problem: PreparedRescalerProblem


def _cpu_if_needed(tensor: torch.Tensor) -> torch.Tensor:
...
def prepare_layer_problem(
    ...
    *,
    ...
    return PreparedLayerProblem(
        ...
        rescaler_problem=prepare_rescaler_problem(target_cross, sigma_x_hat),
    )
...
def quantize_linear_layer(
    ...
    *,
    ...
    search = binary_search_c(
        _cpu_if_needed(problem.target_y),
        _cpu_if_needed(problem.cholesky),
        config.target_rate,
...
def quantize_prepared_layer_problem(
    ...
    *,
    ...
    if config.max_rescaler_iters > 0:
        rescalers = _optimize_diagonal_rescalers_prepared(
            _cpu_if_needed(search.quantization.preliminary_weight),
            problem.rescaler_problem,
            initial_column_scale=_cpu_if_needed(search.quantization.gammas),
            max_iters=config.max_rescaler_iters,
            ridge=config.rescaler_ridge,
        )
    else:
        rescalers = _disabled_rescalers_prepared(
            _cpu_if_needed(search.quantization.preliminary_weight),
            problem.rescaler_problem,
            initial_column_scale=_cpu_if_needed(search.quantization.gammas),
        )
...
def quantize_prepared_layer_problem_objective(
    weight: torch.Tensor,
    problem: PreparedLayerProblem,
    config: LayerQuantizationConfig,
    *,
    ...
    reduced_quantized = rescalers.reconstructed.to(weight.dtype)
    quantized_weight = expand_columns(reduced_quantized, problem.dead_features).to(weight.dtype)
    return ObjectiveQuantizationResult(
        quantized_weight=quantized_weight,
        selected_c=float(search.selected_c),
        effective_rate=float(search.bitrate.final_effective_average_bitwidth),
        entropy_rate=float(search.bitrate.entropy_average_bitwidth),
        huffman_rate=float(search.bitrate.huffman_average_bitwidth),
    )
